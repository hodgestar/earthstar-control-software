...
class PhaseSpinners2(Animation):
    ...
    def ring_render(self, colour_1, colour_2):
        pair = ([colour_1] * self.stripe_width_on) + ([colour_2] * self.stripe_width_off)
        repeats = int(np.ceil(float(self.fc.leds_per_ring) / (self.stripe_width_on + self.stripe_width_off)))
